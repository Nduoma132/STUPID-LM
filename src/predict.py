...
def predict_word(input, model=NB_model):
    ...
    # Get top 5 classes for each sample
    top_k = 5
    top_classes = np.argsort(proba, axis=1)[:, -top_k:][:, ::-1]  # top-k, descending
...
def generate_sentence(words, word_count=50, model=NB_model):
    word_list = words.split(" ")  # convert input to list

    for n in range(word_count):
        main_words = ' '.join(word_list)
        next_word = str(predict_word(main_words, model))
        words += " " + next_word
        word_list = word_list[1:]  # shift window
